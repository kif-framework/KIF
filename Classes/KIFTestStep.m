...
{
    return [self stepToDragViewWithAccessibilityLabel:label direction:direction traits:UIAccessibilityTraitNone];
}
...
{
    return [self stepToDragViewWithAccessibilityLabel:label direction:direction value:nil traits:traits];
}
                                   ...
                                   traits:(UIAccessibilityTraits)traits;
{
    ...
    return [self stepWithDescription:description executionBlock:^(KIFTestStep *step, NSError **error) {
        ...
        CGRect elementFrame = [view.window convertRect:element.accessibilityFrame toView:view];
        CGPoint tappablePointInElement = [view tappablePointInRect:elementFrame];
        CGPoint tappablePointInDirection = [view tappablePointInRect: elementFrame];
        
        // This is mostly redundant of the test in _accessibilityElementWithLabel:
        KIFTestWaitCondition(!isnan(tappablePointInElement.x), error, @"The element with accessibility label %@ is not tappable", label);
       
        if (direction == DRAG_UP ) { 
            tappablePointInDirection.y -= 100;
        } else if ( direction == DRAG_DOWN ) { 
            tappablePointInDirection.y += 100;
        } else if ( direction == DRAG_LEFT ) { 
            tappablePointInDirection.x -= 100;
        } else if ( direction == DRAG_RIGHT ) {
            tappablePointInDirection.x += 100;
        }
        
//        [view tapAtPoint:tappablePointInElement];
        [view dragFromPoint:tappablePointInElement toPoint:tappablePointInDirection];
