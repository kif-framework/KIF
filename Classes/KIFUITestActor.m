...
- (void)tapRowAtIndexPath:(NSIndexPath *)indexPath inTableView:(UITableView *)tableView
{
    ...
    if (!cell) {
        if (indexPath.section >= tableView.numberOfSections) {
            [self failWithError:[NSError KIFErrorWithFormat:@"Section %zd is not found in table view", indexPath.section] stopTest:YES];
        }
        
        if (indexPath.row >= [tableView numberOfRowsInSection:indexPath.section]) {
            [self failWithError:[NSError KIFErrorWithFormat:@"Row %zd is not found in section %zd of table view", indexPath.row, indexPath.section] stopTest:YES];
        }
