...
{
    if (_controller.failureCount != _controller.scenarios.count) {
        KIFLog(@"BEGIN KIF TEST RUN: re-running %d of %d scenarios that failed last time", _controller.failureCount,_controller.scenarios.count);
    } else {
